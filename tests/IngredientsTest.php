...
class IngredientsTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGettingMalts()
    {
        ...
        $this->assertIsArray(
            $malts,
            "Ingredients->getMalts() does not return an array"
        );
        ...
        foreach ($malts as $malt) {
            $this->assertInstanceOf(
                Malt::class,
                $malt,
                'Not all elements in the array returned by "
                . "Ingredients->getMalts() are Malt objects.');
        }
    }
    ...
    public function testGettingTheHops()
    {
        ...
        $this->assertIsArray(
            $hops,
            "Ingredients->getHops() does not return an array"
        );
        ...
        foreach ($hops as $hop) {
            $this->assertInstanceOf(
                ...
                $hop,
                'Not all elements in the array returned by " 
                . "Ingredients->getHops() are Hop objects.');
        }
    }
    ...
    public function testGettingTheYeast()
    {
        ...
        $this->assertIsString(
            $this->ingredientsObject->getYeast(),
            'Ingredients->getYeast() does not return a string.'
        );
        // check that getYeast() returns a non-empty string
        $this->assertNotEmpty(
            $this->ingredientsObject->getYeast(),
            'Ingredients->getYeast() returns an empty string.');
    }
    ...
    public function testGettingIngredientsAsArray()
    {
        ...
        $this->assertIsArray(
            $this->ingredientsObject->toArray(),
            'Ingredients->toArray() does not return an array.'
        );

        // check that the array returned is not empty
        $this->assertNotEmpty(
            $this->ingredientsObject->toArray(),
            "Ingredients->toArray() returns an empty array."
        );
    }

    /**
     * Test building an ingredients object from an array.
     */
